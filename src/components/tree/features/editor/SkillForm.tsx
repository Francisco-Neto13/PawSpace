...
function CharCounter({ current, max }: { current: number; max: number }) {
  ...
  return (
    <span className={`ui-meta font-mono font-bold tabular-nums ${remaining <= 5 ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)]'}`}>
      {remaining}
    </span>
  );
}
      ...
      {!isEditing && initialParentId === undefined && (
        <div>
          ...
          <select
            ...
            className={`${inputClass} appearance-none cursor-pointer uppercase`}
          >
