...
import { updateSkill, savePawSpaceChanges, deleteSkill, addSkill } from '@/app/actions/skills';
...
export function useSkillActions() {
  const { nodes, edges, setNodes, setEdges, refreshPawSpace, originalNodeIds } = usePawSpace();
  ...
  const handleUpdateSkill = useCallback(async (skillId: string, data: Partial<SkillData>) => {
    ...
    if (!result.success) {
      ...
      await refreshPawSpace();
    }
  }, [setNodes, refreshPawSpace]);
  ...
  const handleGlobalSave = useCallback(async () => {
    try {
      ...
      if (toUpdate.length > 0) {
        await savePawSpaceChanges(toUpdate);
      }
