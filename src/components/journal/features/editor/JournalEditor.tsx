      ...
      <div className="shrink-0 border-b border-[var(--border-subtle)] px-4 pb-4 pt-5 md:px-7 md:pt-6">
        <div className="relative">
          <input
            ...
            maxLength={TITLE_MAX}
            className={`w-full bg-transparent text-[var(--text-primary)] text-[1.4rem] font-black outline-none placeholder:text-[var(--text-muted)] tracking-wide pr-16 ${
              isTemporaryEntry ? 'opacity-60 cursor-not-allowed' : ''
            }`}
          />
          {showTitleWarn && (
            <span
              className={`absolute right-0 top-1/2 -translate-y-1/2 ui-meta font-mono font-bold tabular-nums ${
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2">
          ...
          <div className="relative">
            <button
              ...
              className={`h-8 px-2.5 rounded-lg border flex items-center gap-1.5 button-label transition-colors ${
                isTemporaryEntry
              ...
              <>
                ...
                <div className="absolute left-0 top-full z-50 mt-2 w-[min(15rem,calc(100vw-3rem))] overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-base)]">
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 sidebar-meta text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-elevated)] text-left transition-colors font-bold uppercase"
                  >
                    Nota solta
                  </button>
                  ...
                  <div className="max-h-48 overview-scroll-area p-1.5 pt-0.5">
                    {skills.map((item) => (
                      <button
                        ...
                        className="w-full h-9 rounded-lg flex items-center gap-2 px-2.5 sidebar-meta hover:bg-[var(--bg-elevated)] text-left transition-colors border border-transparent hover:border-[var(--border-subtle)]"
          ...
          <div className="ml-auto flex items-center gap-2">
            {isTemporaryEntry ? (
              <span className="flex items-center gap-1.5 button-label text-[var(--text-primary)] animate-pulse">
              ...
              <span className="flex items-center gap-1.5 button-label text-[var(--text-secondary)]">
            ...
            <button
              onClick={onDelete}
              className="h-8 px-2.5 rounded-lg border border-[var(--border-subtle)] flex items-center gap-1.5 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--border-muted)] button-label transition-colors cursor-pointer"
            >
              <Trash2 size={10} />
              Apagar nota
            </button>
