...
function CustomTooltip({ active, payload }: TooltipProps) {
  ...
  return (
    <div className="bg-[var(--bg-base)] border border-[var(--border-muted)] rounded-lg px-3 py-2 text-[10px]">
      <p className="data-label text-[var(--text-primary)]">{d.label}</p>
      <p className="sidebar-meta text-[var(--text-primary)]">{d.count} {d.count === 1 ? 'entrada' : 'entradas'}</p>
      {d.isCurrentMonth && (
        <p className="data-label mt-0.5">mês atual</p>
      )}
      {d.isPeak && d.count > 0 && (
        <p className="data-label mt-0.5">pico</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="h-full overview-card overview-card-hover p-6 relative overflow-hidden group flex flex-col">
      ...
      <div className="flex items-start justify-between mb-1">
        ...
        {totalEntries > 0 && (
          <span
            className="data-label px-2 py-0.5 border"
            style={{
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        <span className="ui-meta uppercase text-[var(--text-secondary)]">pico histórico</span>
      </div>
    </div>
  );
}
