...
function RecentActivityFeed({ initialPage, isBootstrapLoading = false }: RecentActivityFeedProps) {
  ...
  return (
    <div className="h-full max-h-[440px] overview-card overview-card-hover p-6 relative overflow-hidden flex flex-col">
      ...
      <div className="mb-4 ml-3 flex gap-1 overflow-x-auto pb-1 pr-1 shrink-0">
        {([
          ...
          <button
            ...
            className="data-label px-2 py-1 border transition-all duration-200"
            style={{
      ...
      {isLoading ? (
        <p className="ui-meta ml-3">Carregando rastros...</p>
      ) : items.length === 0 ? (
        <p className="ui-meta ml-3">Nenhuma pegada recente por aqui.</p>
      ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="sidebar-meta truncate block group-hover:text-[var(--text-primary)] transition-colors duration-200">
                      {item.title}
                    </span>
                  </div>

                  <span className="ui-meta font-mono shrink-0 group-hover:text-[var(--text-secondary)] transition-colors">
                    {timeAgo(item.date)}
                  </span>
                </div>
              );
      ...
      <div className="mt-4 flex flex-col gap-3 border-t border-[var(--border-subtle)] pt-3 sm:flex-row sm:items-center sm:justify-between shrink-0">
        ...
        <button
          ...
          className="data-label px-2 py-1 border border-[var(--border-muted)] text-[var(--text-secondary)] disabled:text-[var(--text-faint)] disabled:border-[var(--border-subtle)] disabled:cursor-not-allowed"
        >
