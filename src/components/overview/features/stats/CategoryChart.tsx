...
function CustomTooltip({ active, payload }: TooltipProps) {
  ...
  return (
    <div className="bg-[var(--bg-base)] border border-[var(--border-muted)] rounded-lg px-3 py-2 text-[10px] space-y-0.5 shadow-xl">
      <p className="data-label text-[var(--text-primary)]">{d.category}</p>
      <p className="sidebar-meta text-[var(--text-secondary)]">{d.withContent} / {d.total} com material</p>
          ...
          <div className="mt-4 border border-[var(--border-muted)] bg-[var(--bg-surface)] rounded-xl px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                ...
                <p className="text-[var(--text-primary)] font-mono font-black text-base leading-none">{cat.total}</p>
              </div>
            </div>
          </div>
        );
        ...
        <div className="flex items-center gap-4">
          {[['var(--chart-medium)', 'Com material'], ['var(--chart-faint)', 'Total']].map(([color, label]) => (
            <div key={label} className="flex items-center gap-1.5">
              ...
              <span className="ui-meta uppercase">{label}</span>
            </div>
          ))}
        </div>
        <span className="helper-text text-[var(--text-secondary)]">
          Toque em uma barra
        </span>
      </div>
      ...
      {data.length > 1 && (
        <div className="flex gap-3 mt-3">
          <div className="flex-1 border border-[var(--border-subtle)] rounded px-2 py-1.5">
            ...
            <p className="ui-meta font-mono">{bestCategory.pct}% cobertura</p>
          </div>
          <div className="flex-1 border border-[var(--border-subtle)] rounded px-2 py-1.5">
            <p className="data-label mb-0.5">Pedindo atenção</p>
            <p className="sidebar-title truncate">{worstCategory.category}</p>
            <p className="ui-meta font-mono">{worstCategory.pct}% cobertura</p>
          </div>
        </div>
      )}
    </div>
