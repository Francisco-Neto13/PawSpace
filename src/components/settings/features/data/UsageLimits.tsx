...
export default function UsageLimits() {
  const { globalStats, refreshGlobalStats } = usePawSpace();
  ...
  const usageItems = useMemo(() => {
    const totalModules = globalStats.totalSkills;
  ...
  }, [globalStats.totalSkills, globalStats.totalJournalEntries, globalStats.totalLibraryContents]);
          ...
          return (
            <div key={item.label}>
              <div className="flex items-center justify-between mb-2">
                <span className="ui-label text-[var(--text-muted)]">{item.label}</span>
                <span
                  className="ui-meta font-mono font-black"
              ...
              <div className="flex justify-between mt-1">
                <span className="ui-meta uppercase text-[var(--text-faint)]">{item.unit}</span>
                <span className="ui-meta font-mono font-bold" style={{ color: isHigh ? 'rgba(239,68,68,0.7)' : 'var(--text-faint)' }}>
                  {Math.round(pct)}%
                </span>
              </div>
            </div>
