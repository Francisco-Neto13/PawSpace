...
function CharCounter({ current, max }: { current: number; max: number }) {
  ...
  if (!warn) return null;
  return (
    <span className={`ui-meta font-mono font-bold tabular-nums ${remaining <= 10 ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>
      {remaining}
    </span>
  );
}
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-xl rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-strong)] overflow-hidden animate-in zoom-in-95 fade-in duration-200">
        ...
        <div className="relative p-6 md:p-7">
          <div className="mb-6">
            <div className="flex items-center justify-between gap-4 mb-3">
              <div>
                ...
                <h2 className="surface-title">Adicionar material</h2>
              </div>
              <button
                onClick={handleClose}
          ...
          <div className="grid grid-cols-4 gap-1.5 mb-6">
            {TABS.map(tab => (
              <button
                ...
                className={`h-[58px] rounded-xl border flex flex-col items-center justify-center gap-1.5 button-label transition-all duration-200 cursor-pointer
                  ${activeTab === tab.type
          ...
          {submitError && (
            <p className="mt-4 helper-text text-red-400">{submitError}</p>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <button
              type="button"
              onClick={handleClose}
              className="h-11 flex-1 border border-[var(--border-subtle)] rounded-xl text-[var(--text-secondary)] button-label hover:bg-[var(--bg-surface)] hover:text-[var(--text-primary)] hover:border-[var(--border-visible)] transition-all duration-200 cursor-pointer"
            >
              Cancelar
            </button>
            <button
              ...
              className="h-11 flex-1 border border-[var(--border-visible)] rounded-xl bg-[var(--bg-elevated)] text-[var(--text-primary)] button-label hover:bg-[var(--bg-input)] hover:border-[var(--text-secondary)] disabled:opacity-20 disabled:cursor-not-allowed transition-all duration-200 cursor-pointer"
            >
