      ...
      <div className="flex gap-2">
        {(['upload', 'link'] as const).map((m) => (
          <button
            ...
            className={`h-10 flex-1 rounded-xl border button-label transition-all duration-200 cursor-pointer ${
              mode === m
        ...
        <div className="animate-in fade-in duration-300">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              {file && (
                <span className="ui-meta text-[var(--text-secondary)] font-mono">
                  {(file.size / 1024 / 1024).toFixed(2)} MB
                </span>
              )}
            </div>
          </button>
          <p className={`mt-2 ui-meta font-mono ${isOverLimit ? 'text-red-400' : 'text-[var(--text-secondary)]'}`}>
