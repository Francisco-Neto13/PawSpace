...
export async function getRecentActivityPageForUserId(
  ...
  // `limit + 1` por fonte já é suficiente para montar a janela atual
  // e detectar se ainda existe paginação sem puxar linhas demais.
  const fetchTake = Math.min(MAX_LIMIT + OVERFETCH_PADDING, limit + OVERFETCH_PADDING);
