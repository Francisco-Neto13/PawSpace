...
export default function RecentActivityFeed({ nodes }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6 relative overflow-hidden">
      ...
      <p className="text-[9px] text-zinc-600 mb-6 ml-3">diário e módulos atualizados</p>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-1">
          {items.map(item => (
            <div
              key={item.id}
              className="flex items-center gap-3 py-2 border-b border-white/[0.03] last:border-0 group"
            >
