...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-[#0a0a0a] border border-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-wider">
      <p className="text-[#c8b89a]">Nível {d.level}</p>
        ...
        <AreaChart data={data}>
          <defs>
            <linearGradient id="totalGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="#c8b89a" stopOpacity={0.12} />
              <stop offset="95%" stopColor="#c8b89a" stopOpacity={0} />
            </linearGradient>
            <linearGradient id="unlockedGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="#c8b89a" stopOpacity={0.35} />
              <stop offset="95%" stopColor="#c8b89a" stopOpacity={0.02} />
            </linearGradient>
          </defs>
          <XAxis dataKey="level" tick={{ fill: '#52525b', fontSize: 9, fontWeight: 700 }} axisLine={false} tickLine={false} />
          <YAxis hide />
          <Tooltip content={<CustomTooltip />} cursor={{ stroke: 'rgba(200,184,154,0.15)', strokeWidth: 1 }} />
          <Area type="monotone" dataKey="total" stroke="rgba(200,184,154,0.15)" strokeWidth={1} fill="url(#totalGrad)" dot={false} />
          <Area type="monotone" dataKey="unlocked" stroke="#c8b89a" strokeWidth={2} fill="url(#unlockedGrad)" dot={{ fill: '#c8b89a', r: 3, strokeWidth: 0 }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
